...
<template>
  <div class="bigBox" style="height:100vh" v-if="isData">
    <div class="container">
      ...
      <div class="mlr20 p20 br12 bg-ff flex-csb">
        ...
        <div class="flex-center">
          ...
          <div
            class="share flex-dce plr20"
            style="border-left:1px dashed #ccc;"
    ...
    <div class="bg-ff cf flex-cfe p-r">
      <router-link to="shopCar" class="p-a" style="width:1.04rem;top:-.5rem;left:0.2rem;">
      ...
      <span
        class="bg-diy flex-center p20"
        style="background:#FFB301;width:2.2rem"
        @click="shopCar"
      >加入购物车</span>
      <span class="bg-diy flex-center p20" style="width:2.2rem" @click="confirm">立即购买</span>
    </div>
    <div class="mask" v-if="isShow">
      <div class="bottom-fixed bg-ff">
        <div class="flex-cfe p20" @click="isShow=false">
          <img src="../assets/img/ic_quxiao.png" style="width:.4rem;" alt>
        </div>
        ...
        <div class="bg-diy p20 cf flex-center mt20" @click="buy">立即购买</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    init: function() {
      ...
      that.Http.post("Wxsite/Mall/api", {
        ...
        if (res.data.code == 1) {
          ...
          that.isData = 1;
          that.goodData.goods_id = res.data.data.id;
    ...
    getMounthSele: function() {
      const that = this;
      that.Http.post("Wxsite/Mall/api", {
        api_name: "get_month_sale",
    ...
    buy: function() {
      ...
      if (that.goodData.id == "") {
        ...
        if (that.isType == 1) {
          let that = this;
          that.Http.post("Wxsite/Mall/api", {
            api_name: "add_goods_cart",
            goods_id: that.goodData.goods_id,
            goods_spec_id: that.goodData.id,
          ...
          that.$router.push({
            path: "/shopConfirm",
            query: {
              spec: that.goodData.id + "_" + that.goodData.num
            }
          });
        }
      }
