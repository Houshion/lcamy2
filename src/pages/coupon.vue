  ...
  <div class="bigBox">
    <div class="container" v-if="isData">
      <div v-if="coupon.length>0">
        <div
          class="m20 br12 bg-ff flex-csb of-hidden"
          style="heigth:2.2rem;"
          v-for="(item,index) in coupon"
          :key="index"
          v-if="item.status_name!='已使用'"
        >
          <ul class="flex-dcf p30">
            <li :class="item.status_name=='已使用'?'f44 c9':'f44 brown'">{{item.status_name}}</li>
            <li class="c9 mt60 f25">有效日期：{{item.ctime|timeStr('Y-m-d')}} ~ {{item.etime}}</li>
          </ul>
          <div
            :class="item.state?'used rightBox flex-center p-r':'rightBox flex-center p-r p30 noUsed'"
          >
